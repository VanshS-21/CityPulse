...
class CredentialsManager {
  ...
  async setupCredentials() {
    ...
    if (!fs.existsSync(this.credentialsPath)) {
      ...
      console.log('Please ensure citypulse-credentials.env exists in the credentials folder.');
      return;
    }
    ...
    const setupFirebase = await this.askQuestion('Setup Firebase credentials? (y/n): ');
    if (setupFirebase.toLowerCase() === 'y') {
      await this.setupFirebaseCredentials();
    }

    const setupGCP = await this.askQuestion('Setup Google Cloud credentials? (y/n): ');
    if (setupGCP.toLowerCase() === 'y') {
      await this.setupGCPCredentials();
    }

    console.log('\n✅ Credentials setup completed!');
    console.log('📝 Please review and update the credentials file with your actual values.');
  }
  ...
  async setupFirebaseCredentials() {
    ...
    await this.askQuestion('Press Enter when you have the Firebase config ready...');
  }
  ...
  async generateSecrets() {
    ...
    if (save.toLowerCase() === 'y') {
      ...
      Object.entries(secrets).forEach(([key, value]) => {
        ...
        credentialsContent = credentialsContent.replace(regex, `${key}="${value}"`);
      });
