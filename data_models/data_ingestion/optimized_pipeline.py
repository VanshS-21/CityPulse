...
class LocationHashGenerator(beam.DoFn):
    ...
    def process(self, element):
        ...
        try:
            if (
                "location" in element
                and "latitude" in element["location"]
                and "longitude" in element["location"]
            ):
        ...
        try:
            ...
            if "created_at" in element:
                if isinstance(element["created_at"], str):
                    element["created_at"] = datetime.fromisoformat(
                        element["created_at"].replace("Z", "+00:00")
                    )
...
class OptimizedCityPulsePipeline:
    ...
    def __init__(self, options: OptimizedPipelineOptions):
        ...
        self.pipeline_metrics = {
            "total_processed": Metrics.counter(
                "OptimizedPipeline", "total_elements_processed"
            ),
            "successful_writes": Metrics.counter(
                "OptimizedPipeline", "successful_writes"
            ),
            "dead_letter_events": Metrics.counter(
                "OptimizedPipeline", "dead_letter_events"
            ),
        }
    ...
    def create_pipeline(self, pipeline_options: PipelineOptions):
        ...
        with beam.Pipeline(options=pipeline_options) as pipeline:
            ...
            raw_events = (
                pipeline
                | "ReadFromPubSub"
                >> beam.io.ReadFromPubSub(topic=self.options.input_topic)
                | "ParseJSON" >> beam.Map(json.loads)
                | "AddTimestamps" >> beam.Map(self.add_processing_timestamp)
            )

            # Apply adaptive windowing strategy
            windowed_events = raw_events | "ApplyWindowing" >> beam.WindowInto(
            ...
            validated_events = windowed_events | "ValidateData" >> RetryableTransform(
                OptimizedDataValidator(), max_retries=self.options.max_retries
            )
            ...
            enriched_events = (
                ...
                | "GenerateEventID"
                >> beam.ParDo(EventIDGenerator()).with_outputs(
                    "dead_letter", main="main"
                )
                | "GenerateLocationHash"
                >> beam.ParDo(LocationHashGenerator()).with_outputs(
                    "dead_letter", main="main"
                )
            )
            ...
            ai_processed_events = (
                ...
                | "BatchedAIProcessing"
                >> beam.ParDo(BatchedAIProcessor(self.options.batch_size)).with_outputs(
                    "dead_letter", main="main"
                )
            )
            ...
            bigquery_ready = (
                ...
                | "PrepareForBigQuery"
                >> beam.ParDo(
                    OptimizedBigQueryWriter(self.options.output_table)
            ...
            (
                ...
                >> beam.Map(lambda x: self.pipeline_metrics["successful_writes"].inc())
            )
            ...
            all_dead_letters = (
                ...
                >> beam.ParDo(
                    EnhancedDeadLetterHandler(
                        "OptimizedCityPulsePipeline", self.options.error_table
                    )
                )
                | "WriteDeadLetters"
                >> beam.io.WriteToBigQuery(
                ...
                | "CountDeadLetters"
                >> beam.Map(lambda x: self.pipeline_metrics["dead_letter_events"].inc())
            )
            ...
            (
                raw_events
                | "CountTotalProcessed"
                >> beam.Map(lambda x: self.pipeline_metrics["total_processed"].inc())
            )
...
def run_optimized_pipeline(argv=None):
    ...
    # Configure optimized pipeline options using shared config
    optimized_options = OptimizedPipelineOptions(
        input_topic=f"projects/{config.project_id}/topics/{config.pubsub.citizen_reports_topic}",
        output_table=f"{config.project_id}:{config.database.bigquery_dataset}.events_optimized",
        error_table=f"{config.project_id}:{config.database.bigquery_dataset}.pipeline_errors",
        batch_size=100,
        window_size=60,
        max_retries=3,
