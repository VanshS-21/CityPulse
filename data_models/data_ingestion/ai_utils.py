...
) -> tuple[GenerativeModel, ImageGenerationModel]:
...
def generate_icon(
    storage_client, bucket_name: str, category: str, event_id: str
) -> Optional[str]:
    """
...
def create_prompt(text: str, image_url: Optional[str] = None) -> list[Part]:
    ...
    """
    prompt_parts = []
    prompt_text = config.ai.analysis_prompt.format(text_to_process=text)
