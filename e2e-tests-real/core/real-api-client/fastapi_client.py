...
class RealFastAPIClient:
    ...
    async def make_request(
        ...
        # Prepare URL
        url = f"/v1{endpoint}" if not endpoint.startswith("/") else f"/v1{endpoint}"
    ...
    async def test_events_api(self) -> Dict[str, Any]:
        ...
        if self.auth_token:
            try:
                ...
                response = await self.make_request("POST", "/events", data=new_event)
                test_result = {
                    "name": "POST /events",
    ...
    async def test_analytics_api(self) -> Dict[str, Any]:
        ...
        # Test 1: Get KPIs
        try:
            response = await self.make_request("GET", "/analytics/kpis")
            test_result = {
    ...
    async def cleanup_test_data(self):
        ...
        if "created_event_id" in self.session_data and self.auth_token:
            try:
                ...
                response = await self.make_request("DELETE", f"/events/{event_id}")
