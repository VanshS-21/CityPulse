...
class ProcessWithAI(beam.DoFn):
    ...
    def setup(self):
        ...
        self.model = GenerativeModel(config.ai.model_name)
        self.image_gen_model = ImageGenerationModel.from_pretrained(
            "imagegeneration@006"
        )
        self.storage_client = storage.Client()
        self.bucket = self.storage_client.bucket(config.storage.generated_images_bucket)
    ...
    async def _get_prompt_parts(self, text_to_process, image_url):
        ...
        prompt_parts = []
        prompt_text = config.ai.analysis_prompt.format(text_to_process=text_to_process)
        ...
        if image_url:
            try:
                ...
                prompt_parts.append(
                    Part.from_data(await response.read(), mime_type=mime_type)
                )
    ...
    def _parse_ai_response(self, response):
        ...
        try:
            ...
            logging.error(
                "Error parsing AI response: %s. Response: %s", e, response.text
            )
            return {
    ...
    async def _generate_and_upload_icon(self, category, event_id):
        ...
        try:
            prompt = config.ai.icon_prompt.format(category=category.lower())
            ...
            logging.error(
                "Error generating/uploading icon for event %s: %s", event_id, e
            )
        return None
        ...
        try:
            ...
            if ai_insights:
                ...
                icon_url = await self._handle_icon_generation(
                    ai_insights.get("category"),
                    element.get("event_id", element.get("id", "unknown")),
                )
                if icon_url:
            ...
            logging.error(
                "Error processing element %s with AI: %s",
                element.get("event_id", element.get("id", "unknown")),
                e,
            )
