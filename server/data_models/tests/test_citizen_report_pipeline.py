...
from data_models.data_ingestion.citizen_report_pipeline import (
    ...
    CitizenReportPipeline,
)
from data_models.firestore_models.event import Event
from data_models.tests.common import MOCK_EVENT_DATA, BaseBeamTest
...
class ProcessMultimediaTest(BaseBeamTest):
    ...
    def test_process_multimedia_success(
        self, mock_requests_session, mock_storage_client
    ):
        ...
        event_with_media["metadata"] = {"media_url": "http://example.com/image.jpg"}
    ...
    def test_process_multimedia_download_failure(self, mock_requests_session):
        ...
        results = (
            self.pipeline
            | beam.Create([event])
            | beam.ParDo(ProcessMultimedia(bucket_name="test-bucket")).with_outputs(
...
class CitizenReportPipelineTest(BaseBeamTest):
    ...
    def test_integrated_pipeline(
        ...
        mock_mm_storage_client,
        mock_ai_storage_client,
        ...
        mock_mm_storage_client.return_value.bucket.return_value.blob.return_value = (
            mock_mm_blob
        )
